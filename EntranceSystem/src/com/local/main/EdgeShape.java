...
class EdgeShape {
    ...
    private static final int[] polarDistance = new int[polarCacheRadius*polarCacheRadius];
    private static final double[] polarAngle = new double[polarCacheRadius*polarCacheRadius];
    final int length;
    ...
    static {
        for (int y = 0; y < polarCacheRadius; ++y)
            for (int x = 0; x < polarCacheRadius; ++x) {
                polarDistance[y * polarCacheRadius + x] = (int)Math.round(Math.sqrt((x*x) + (y*y)));
                if (y > 0 || x > 0)
